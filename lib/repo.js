...
// A bare name (not a URL, not a path) that should be looked up in the
// Repositories database to find its Default Location.
function isRepoName(value) {
  const v = (value || "").trim();
  if (!v) return false;
  if (isRepoUrl(v)) return false;
  // Path-like: starts with ~, /, ., \ or contains / or \
...
module.exports = { resolveRepoDir, resolveWorkingDir, isRepoUrl, isRepoName, repoNameFromUrl, expandHome };
...
if (require.main === module) {
  ...
  assert.equal(isRepoName("https://github.com/you/telemetry"), false);
  assert.equal(isRepoName("/tmp/x"), false);
  assert.equal(isRepoName("~/projects/x"), false);
  assert.equal(isRepoName("./relative"), false);
  assert.equal(isRepoName("my-project"), true);
  assert.equal(isRepoName(""), false);
