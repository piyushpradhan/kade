...
function orchestrationPreamble({ maxTasks = 20 } = {}) {
  return [
    ...
    "",
    "If — and only if — you cannot proceed without a decision that only the human can make, do NOT guess. End your reply with exactly one <needs-decision> block stating the question and the concrete options:",
    "<needs-decision>",
    "Your question, with the options you're choosing between.",
    "</needs-decision>",
    "The ticket then moves to Review; when the human answers and moves it back to In Progress, you resume this same session with their decision. Emit at most one of <task-plan> or <needs-decision> — a decision block wins and pauses the task.",
    "",
    "Task:",
  ].join("\n");
}
